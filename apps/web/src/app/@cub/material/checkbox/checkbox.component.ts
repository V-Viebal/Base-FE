...
export const createCheckbox = _.memoize(
	function(
...
export class CUBCheckboxComponent
	...
	@Input() public onBeforeToggle: () => boolean | Promise<boolean>;
	...
	ngOnInit() {
		this.checkboxDom
			= createCheckbox( !!this.value, this.size );
	}
	...
	override writeValue( value: boolean ) {
		...
		this.checkboxDom
			= createCheckbox( !!value, this.size );
	}
