...
export class LeaderLineNew extends LeaderLine {
	...
	set scaleRatio( ratio: number ) {
		...
		this.addLabel( this._label, ratio );
	}
	...
	get options(): LeaderLineNew.Options {
	...
	constructor( options: LeaderLineNew.Options ) {
		...
		this._svg = document.querySelector(
	...
	public override position() {
		try {
			if ( !document.body.contains( this.start as Element )
				|| !document.body.contains( this.end as Element ) ) {
				throw new Error();
			}
	...
	protected addLabel(
		...
		scaleRatio: number = this._scaleRatio
	) {
	...
	protected addRoundedCornersToGridPath(
		cornerRadius: number = this._cornerRadius
	) {
		if ( !cornerRadius
			|| this.path !== 'grid' ) {
			return;
