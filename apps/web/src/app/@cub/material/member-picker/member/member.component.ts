...
export class CUBMemberComponent implements OnChanges{
	...
	@Input() @DefaultValue() @CoerceBoolean()
	public displayMemberAvatar: boolean = true;
	@Input() @CoerceBoolean()
	...
	ngOnChanges( changes: SimpleChanges ) {
		if ( changes.member?.currentValue?.status
				!== this.MEMBER_STATUS.DELETED ) {
			return;
		}
		...
		this.member.avatar = {
			...this.member.avatar,
			mode: 'photo',
			photo: 'assets/images/icons/user-deleted.svg',
		};
	}
}
