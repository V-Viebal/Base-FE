...
export type CUBDropdownMenuSize
	= CUBMenuSizeConfig;
export type DropdownSearchConfig
	= {
		placeholder: string;
...
export class CUBDropdownComponent
	...
	@Input() public displayItemError: boolean;
	...
	@Input() public errorsMessage: Map<string, ObjectType>;
	...
	@Input() public searchConfig: DropdownSearchConfig;
	@Input() public menuSize: CUBDropdownMenuSize;
	@Input() public beforeSelectItem:
		( item: CUBDropdownItemComponent ) => boolean | Promise<boolean>;
	...
	ngOnDestroy(): void {
		this.dropdownInput.formField.control.markAsUntouched();
		this.dropdownInput.formField.control.markAsPristine();
	}
	...
	private _display() {
		const map: Map<any, CUBDropdownItemComponent>
			= new Map<any, CUBDropdownItemComponent>();
