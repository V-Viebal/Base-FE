...
export class ControlErrorsPipe implements PipeTransform {
	...
	private _getFieldErrors(
		...
		const validKeyLookup: ObjectType
			= ControlErrorsPipe.TRANSLATE_KEY_LOOKUP;
		...
		return _.map(
			...
			( errorType: string ) => {
				const errorKey: string
					= _.toUpper( validKeyLookup[ errorType ] );
				...
				const translateKey: string
					= `CORE.VALIDATION_MESSAGE.${errorKey}`;
				let value: string;
