...
export class HighlightStateDirective implements OnChanges, OnDestroy {
	...
	@Input() @DefaultValue()
	public highlightType: 'background' | 'border' = 'background';
	@Input() @DefaultValue()
	public highlightColor: string = COLOR.HIGHLIGHT;
	@Input() @CoerceBoolean()
	public highlightState: boolean;
	@Input() @CoerceBoolean()
	public highlightDisabled: boolean;
	@Input() @CoerceBoolean()
	public autoUnhighlight: boolean;
	@Input() @DefaultValue()
	public autoUnhighlightDelay: number = 3000;

	@Output() public highlightStateChange: EventEmitter<boolean>
	...
	private _autoUnhighlightDebounce: ReturnType<typeof _.debounce>
		= _.debounce(
			this.unsetHighlight.bind( this ),
			this.autoUnhighlightDelay
		);
	...
	ngOnChanges( changes: SimpleChanges ) {
		...
		this.highlightState
			? this._setHighlightDebounce( this.autoUnhighlight )
			: this._unsetHighlightDebounce();
	}
